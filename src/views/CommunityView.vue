    ...
    <div v-if="paging.total_list_cnt > 0" class="pagination w3-bar w3-padding-16 w3-small">
      <span class="pg">
        ...
        <a
          ...
          >&gt;</a
        >
        <a href="javascript:;" class="last w3-button w3-border" @click="fnPage(`${paging.total_page_cnt}`)">&gt;&gt;</a>
      </span>
    </div>
      ...
      await axios
        ...
        .then(response => {
          ...
          for (let i in this.data) {
            let a = response.data.data.list[i].title
            this.score.push(a)
          }
          for (let i in this.data) {
            let a = response.data.data.list[i].nick
            this.nick.push(a)
          }
          // for (let i in this.data) {
          ...
          // }
          for (let i in this.data) {
            let a = response.data.data.list[i].clickCount
          ...
          for (let i in this.data) {
            let a = response.data.data.list[i].clickLike
            this.date.push(a.substr(0, 10))
          }
        })
        .catch(error => {
